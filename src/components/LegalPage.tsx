...
const business = {
  name: import.meta.env.VITE_BUSINESS_NAME || 'Самозанятый — данные будут опубликованы перед запуском платежей',
  inn: import.meta.env.VITE_BUSINESS_INN || 'ИНН будет опубликован перед запуском платежей',
  email: import.meta.env.VITE_SUPPORT_EMAIL || 'support@clientlens.ru',
  phone: import.meta.env.VITE_SUPPORT_PHONE || 'Способ связи будет опубликован перед запуском платежей',
};
...
function content(kind: LegalKind) {
  ...
  if (kind === 'contacts') return <><p>Исполнитель: {business.name}</p><p>ИНН: {business.inn}</p><p>Email: <a href={`mailto:${business.email}`}>{business.email}</a></p><p>Связь: {business.phone}</p><p>Услуга предоставляется онлайн: после оплаты баланс пополняется, а результат анализа становится доступен в аккаунте.</p></>;
  ...
  if (kind === 'privacy') return <><p>Мы обрабатываем email, ник, выбранный аватар и данные, которые вы сами отправляете для анализа профиля. Отчёты хранятся в вашем аккаунте и доступны только вам через правила доступа базы данных.</p><p>Для AI-анализа данные профиля передаются OpenAI. Не отправляйте в сервис пароли, платёжные данные, паспортные данные или другую чувствительную информацию.</p><p>Чтобы запросить удаление аккаунта и отчётов, напишите на {business.email}.</p></>;
