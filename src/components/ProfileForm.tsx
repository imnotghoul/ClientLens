...
export function ProfileForm({ onAnalyze, onDemo = () => undefined, notice }: {
  ...
  return <form className="audit-form dark-form" onSubmit={submit}>
    ...
    {mode === 'competitive' && <section className="competitors"><b>Конкуренты для сравнения</b><p>Добавьте до трёх публичных ссылок. Если страница недоступна, заполните данные конкурента вручную.</p>{competitors.map((item, index) => <div className="competitor" key={index}><input placeholder="Имя конкурента" value={item.name} onChange={(event) => updateCompetitor(index, 'name', event.target.value)} /><input placeholder="https://…" value={item.profileUrl ?? ''} onChange={(event) => updateCompetitor(index, 'profileUrl', event.target.value)} /><input placeholder="Название услуги или предложение" value={item.headline} onChange={(event) => updateCompetitor(index, 'headline', event.target.value)} /><input placeholder="Цена, отзывы или кейсы" value={item.price} onChange={(event) => updateCompetitor(index, 'price', event.target.value)} /><button type="button" className="remove-competitor" aria-label="Удалить конкурента" title="Удалить конкурента" onClick={() => removeCompetitor(index)}>×</button></div>)}{competitors.length < 3 && <button className="text-button" type="button" onClick={() => setCompetitors((old) => [...old, blankCompetitor()])}>+ Добавить конкурента</button>}</section>}
