...
export function AccountPanel({ mode = 'profile', initialScreen = 'login', onProfileChanged }: AccountPanelProps) {
  ...
  const uploadAvatar = async (file: File | undefined) => {
    ...
    let prepared: File;
    try { prepared = await prepareAvatarFile(file); } catch (error) { return setMessage(error instanceof Error ? error.message : 'Не удалось обработать аватар.'); }
    const path = `${session.user.id}/avatar.${avatarFileExtension(prepared.type)}`;
    const { error } = await supabase.storage.from('avatars').upload(path, prepared, { upsert: true, contentType: prepared.type });
